...
import { casinoSchema } from "@ssp/utils"

import { clientApi } from "@/trpc/client"
import { CasinoEditForm } from "./casino-edit-form"
...
export function DataTableRowActions<TData>({ row }: DataTableRowActionsProps<TData>): React.JSX.Element {
  const casino = casinoSchema.parse(row.original)
  ...
  const deleteBonus = clientApi.casino.delete.useMutation({
    onSuccess: () => {
      toast.success("The bonus has been deleted with success. 🎉")
    ...
    if (confirm) {
      startTransition(async () => {
        try {
          ...
          await deleteBonus.mutateAsync({ id: casino.id })
        } catch (error) {
    ...
    <div>
      ...
      <Dialog open={openDialogEdit} onOpenChange={setOpenDialogEdit}>
        <DialogContent className='no-scrollbar  max-h-[800px] max-w-6xl overflow-auto p-10'>
          ...
          <Separator className='my-6' />
          <CasinoEditForm setOpenDialogEdit={setOpenDialogEdit} casinoId={casino.id} />
        </DialogContent>
      </Dialog>
    </div>
  )
